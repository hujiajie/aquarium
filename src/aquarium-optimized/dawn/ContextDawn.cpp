...
ContextDawn::ContextDawn(BACKENDTYPE backendType)
    : queue(nullptr),
      ...
      mPipeline(nullptr),
      mBindGroup(nullptr),
      mPreferredSwapChainFormat(dawn::TextureFormat::BGRA8Unorm),
      mEnableMSAA(false),
      mappedData(nullptr)
{
...
{
#if __EMSCRIPTEN__
    if (backend != BACKENDTYPE::BACKENDTYPEWEBGPU) {
        return false;
    }

    mDevice = dawn::Device::Acquire(emscripten_webgpu_get_device());
...
#if __EMSCRIPTEN__
    mBackbuffer     = dawn::Texture::Acquire(emscripten_webgpu_get_current_texture());
#else
