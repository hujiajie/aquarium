                                ...
                                const void *pixels)
{
  ...
  wgpu::CommandBuffer command =
      copyBufferToBuffer(result.buffer, 0, buffer, 0, size);
...
void ContextDawn::initGeneralResources(Aquarium *aquarium)
{
  ...
  setBufferData(mLightBuffer, 0, sizeof(LightUniforms),
...
{
  updateBufferData(
      mLightWorldPositionBuffer, &aquarium->lightWorldPositionUniform,
